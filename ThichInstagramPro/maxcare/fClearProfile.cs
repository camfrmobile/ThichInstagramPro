...
namespace maxcare
{
	public class fClearProfile : Form
	{
					...
					while (num < lstProfileRac.Count)
					{
						if (iThread < maxThread)
						{
							...
							new Thread((ThreadStart)delegate
							{
								try
								{
									string path = FileHelper.GetPathToCurrentFolder() + "\\profile\\" + lstProfileRac[row];
		...
		private List<string> GetListUidFromDatabase()
		{
			...
			try
			{
				DataTable allAccountFromDatabase = CommonSQL.GetAllAccountFromDatabase("uid");
		...
		private void OnLoaded(object sender, EventArgs e)
		{
			...
			new Thread((ThreadStart)delegate
			{
				try
				{
					List<string> list = Directory.GetDirectories(FileHelper.GetPathToCurrentFolder() + "\\profile").ToList();
