...
namespace maxcare
{
	public class fUpAvatar : Form
	{
		...
		private void ExcuteOneThread_Chrome(int indexRow, int indexPos, string idKichBan, string profilePath)
		{
			...
			if (num2 == 1)
			{
				text10 = GetStatusAccount(indexRow);
			}
			try
			{
				if (chrome?.CheckIsLive() ?? false)
				{
					int num7 = rd.Next(setting_general.GetValueInt("nudDelayCloseChromeFrom"), setting_general.GetValueInt("nudDelayCloseChromeTo") + 1);
					if (num7 > 0)
					{
						int tickCount2 = Environment.TickCount;
						while ((Environment.TickCount - tickCount2) / 1000 - num7 < 0)
						{
							if (!isStop)
							{
								SetStatusAccount(indexRow, text2 + Language.GetValue("Đóng tri\u0300nh duyê\u0323t sau {time}s...").Replace("{time}", (num7 - (Environment.TickCount - tickCount2) / 1000).ToString()));
								MCommon.Common.DelayTime(0.5);
								continue;
							}
							SetStatusAccount(indexRow, text2 + Language.GetValue("Đã dừng!"));
			...
			if (!(text12 == ""))
			{
				SetStatusAccount(indexRow, text2 + text10 + (flag ? "- Facebook blocked" : ""));
			}
			else
			{
				...
				new Thread((ThreadStart)delegate
				{
					try
					{
						...
						while (num2 < num)
						{
							...
							if (setting_InteractGeneral.GetValueBool("ckbRepeatAll"))
							{
								text2 = ((num > 1) ? string.Format(Language.GetValue("Lượt {0}/{1}. "), num2 + 1, num) : "");
								if (setting_InteractGeneral.GetValueBool("RepeatAllVIP"))
								{
									list = setting_InteractGeneral.GetValueList("lstIdKichBan");
									if (setting_InteractGeneral.GetValueBool("ckbRandomKichBan"))
							...
							while (true)
							{
								if (num3 < list.Count && !isStop)
								{
									...
									if (text2 != "")
									{
										ShowTrangThai(text2 + string.Format(Language.GetValue("Kịch bản") + ": {0}...", InteractSQL.GetTenKichBanById(text)));
									}
									if (setting_InteractGeneral.GetValueBool("ckbRandomThuTuTaiKhoan"))
									{
										dtgvAcc.Invoke((MethodInvoker)delegate
									...
									if (!isStop)
									{
										if (num3 + 1 < list.Count)
										{
											...
											{
												ShowTrangThai(text2 + string.Format(Language.GetValue("Chạy kịch bản tiếp theo sau {time} giây...").Replace("{time}", (num9 - (Environment.TickCount - tickCount) / 1000).ToString())));
												MCommon.Common.DelayTime(0.5);
												if (isStop)
												{
													break;
								...
								if (num2 + 1 < num)
								{
									...
									{
										ShowTrangThai(text2 + string.Format(Language.GetValue("Chạy lượt tiếp theo sau {time} giây...").Replace("{time}", (num10 - (Environment.TickCount - tickCount2) / 1000).ToString())));
										MCommon.Common.DelayTime(0.5);
										if (isStop)
										{
											break;
			...
			try
			{
				...
				else
				{
					while (true)
					{
						...
						SetStatusAccount(indexRow, text2 + Language.GetValue("Đang") + " Up Avatar...");
						bool flag8 = false;
						int num7 = 0;
						List<string> list = new List<string>();
						num7 = 1;
						...
						while (true)
						{
							...
							while (flag8)
							{
								...
								case 2:
									...
									{
										device.Swipe(163, 57, 162, 33);
										device.DelayTime(2.0);
										list = device.GetListBoundsByText("\"photo thumbnail\"", "", 10);
		...
		{
			setting_general = new JSON_Settings("configGeneral");
			setting_InteractGeneral = new JSON_Settings("configUpAvatar");
		}
		...
		{
			MCommon.Common.ShowForm(new fUpAvatar_CauHinh());
		}
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(maxcare.fUpAvatar));
			...
			copyToolStripMenuItem.DropDownItems.AddRange(new System.Windows.Forms.ToolStripItem[9] { uidToolStripMenuItem, passToolStripMenuItem, cookieToolStripMenuItem, uidPassToolStripMenuItem, uidPass2FaToolStripMenuItem, mailPassMailToolStripMenuItem, useragentToolStripMenuItem1, proxyToolStripMenuItem1, đinhDangKhacToolStripMenuItem });
			...
			mailPassMailToolStripMenuItem.Size = new System.Drawing.Size(163, 22);
			...
			bunifuCustomLabel1.Size = new System.Drawing.Size(277, 16);
			bunifuCustomLabel1.TabIndex = 3;
			bunifuCustomLabel1.Text = "Max Instagram Pro - Ti\u0301nh năng Up Avatar";
