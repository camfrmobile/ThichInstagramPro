...
namespace maxcare
{
	public class fDangBai : Form
	{
		...
		private object lock_GetAnh = new object();

		private List<Device> lstDevice = null;

		public bool isResetAdb = false;
		...
		private void DelAllFileInFolder(string pathFolder)
		{
			try
			{
		...
		private void RunByChrome()
		{
			try
			{
				...
				if (setting_InteractGeneral.GetValueBool("ckbVanBan"))
				{
					lstContent = setting_InteractGeneral.GetValueList("txtNoiDung");
					if (lstContent.Count == 0)
					{
						MessageBoxHelper.ShowMessageBox(Language.GetValue("Vui lòng nhập nội dung!"), 3);
						return;
					}
				}
				string value = setting_InteractGeneral.GetValue("txtPathPicChrome");
				if (Directory.GetFiles(value, "*.*", SearchOption.AllDirectories).ToList().Count == 0)
				{
					MessageBoxHelper.ShowMessageBox(Language.GetValue("Vui lòng nhập thêm ảnh!"), 3);
					return;
				}
				...
				new Thread((ThreadStart)delegate
				{
					...
					if (setting_InteractGeneral.GetValueBool("ckbVanBan"))
					{
						setting_InteractGeneral.Update("txtNoiDung", lstContent);
		...
		private void ExcuteOneThread_Chrome(int indexRow, int indexPos, string idKichBan, string profilePath)
		{
			...
			string cellAccount3 = GetCellAccount(indexRow, "cCookies");
			GetCellAccount(indexRow, "cToken");
			if (text4 == "")
			{
				text4 = CommonIG.GetUidFromCookie(cellAccount3);
			}
			try
			{
				if (isStop)
				...
				else
				{
					while (true)
					{
						...
						{
							string text6 = profilePath + "\\" + cellAccount;
							string pathTo = profilePath + "\\" + text4;
							if (!MCommon.Common.MoveFolder(text6, pathTo) && MCommon.Common.CopyFolder(text6, pathTo))
							{
								MCommon.Common.DeleteFolder(text6);
							}
						}
						string text7 = "";
						text7 = ((!(text4 != "")) ? (profilePath + "\\" + cellAccount) : (profilePath + "\\" + text4));
						if (!setting_general.GetValueBool("ckbCreateProfile") && !Directory.Exists(text7))
						{
							text7 = "";
						}
						chrome = new Chrome
						{
							...
							UserAgent = SetupFolder.GetUseragentIPhone(rd)
						};
						...
						while (true)
						{
							if (chrome.Open())
							{
								...
								{
									string text9 = LoginInstagram(chrome, indexRow, setting_general.GetValueInt("typeLogin"), text2);
									switch (text9)
									{
									default:
								...
								if (text4.Trim() == "")
								{
									cellAccount3 = chrome.GetCookieFromChrome("instagram");
									text4 = CommonIG.GetUidFromCookie(cellAccount3);
									SetCellAccount(indexRow, "cUid", text4);
									CommonSQL.UpdateFieldToAccount(cellAccount, "uid", text4);
									SetCellAccount(indexRow, "cCookies", cellAccount3);
									CommonSQL.UpdateFieldToAccount(cellAccount, "cookie1", cellAccount3);
								}
								if (MCommon.Common.IsValidMail(cellAccount2) || MCommon.Common.IsNumber(cellAccount2))
								{
									...
									if (value.Trim() != "")
									{
										...
										CommonSQL.UpdateFieldToAccount(cellAccount, "phone", cellAccount2);
										cellAccount2 = value;
									}
								}
								string value2 = setting_InteractGeneral.GetValue("txtPathPicChrome");
								List<string> list = Directory.GetFiles(value2, "*.*", SearchOption.AllDirectories).ToList();
								List<string> list2 = CloneList(list);
								List<string> list3 = CloneList(lstContent);
								int num7 = rd.Next(setting_InteractGeneral.GetValueInt("nudSoLuongBaiFrom", 1), setting_InteractGeneral.GetValueInt("nudSoLuongBaiTo", 1) + 1);
								int valueInt = setting_InteractGeneral.GetValueInt("nudKhoangCachFrom", 5);
								int valueInt2 = setting_InteractGeneral.GetValueInt("nudKhoangCachTo", 10);
								bool valueBool = setting_InteractGeneral.GetValueBool("ckbVanBan");
								bool valueBool2 = setting_InteractGeneral.GetValueBool("ckbXoaNguyenLieuDaDung");
								bool valueBool3 = setting_InteractGeneral.GetValueBool("ckbXoaAnhDaDang");
								string text10 = "";
								string text11 = "";
								int num8 = 0;
								while (true)
								{
									if (num8 < num7)
									{
										chrome.GotoURLIfNotExist("https://www.instagram.com/");
										DelayThaoTacNho();
										chrome.ExecuteScript("HTMLInputElement.prototype.click = function() { if(this.type !== 'file') HTMLElement.prototype.click.call(this); };");
										if (chrome.Click(4, "[data-testid=\"new-post-button\"]") != 0)
										{
											if (chrome.CheckExistElement("section>nav:nth-child(4) form input", 10.0) != 1)
											{
												goto IL_1c92;
											}
											if (valueBool3)
											{
												lock (lock_GetAnh)
												{
													list = Directory.GetFiles(value2, "*.*", SearchOption.AllDirectories).ToList();
													if (list.Count == 0)
													{
														SetStatusAccount(indexRow, text2 + Language.GetValue("Vui lòng nhập thêm ảnh!"));
														isStop = true;
														num2 = 1;
														break;
													}
											...
											text10 = list2[rd.Next(0, list2.Count)];
											list2.Remove(text10);
											goto IL_1a27;
										}
									}
									...
									continue;
									IL_1bb9:
									text11 = MCommon.Common.SpinText(text11, rd);
									if (chrome.CheckExistElement("section textarea", 5.0) == 1)
									{
										chrome.SendKeys(4, "section textarea", text11);
										goto IL_1c0c;
									}
									goto IL_1ca7;
									IL_1a27:
									chrome.SendKeys(4, "section>nav:nth-child(4) form input", text10);
									if (chrome.CheckExistElement("button .createSpriteExpand", 30.0) == 0)
									{
										goto IL_1c92;
									}
									if (valueBool3)
									{
										FileHelper.DeleteFile(text10);
									}
									if (chrome.CheckExistElement("section header div:nth-child(3)>button", 5.0) == 1)
									{
										chrome.ExecuteScript("document.querySelectorAll('section header div:nth-child(3)>button')[0].click()");
										if (valueBool)
										{
											if (valueBool2)
											{
												lock (lstContent)
												{
													if (lstContent.Count == 0)
													{
														SetStatusAccount(indexRow, text2 + Language.GetValue("Vui lòng nhập thêm nội dung!"));
														isStop = true;
														num2 = 1;
														break;
													}
													text11 = lstContent[rd.Next(0, lstContent.Count())];
													lstContent.Remove(text11);
													goto IL_1bb9;
												}
											}
											if (list3.Count == 0)
											{
												list3 = CloneList(lstContent);
											}
											text11 = list3[rd.Next(0, list3.Count())];
											list3.Remove(text11);
											goto IL_1bb9;
										}
										goto IL_1c0c;
									...
									chrome.Click(4, "section header div:nth-child(3)>button");
									for (int i = 0; i < 30; i++)
									{
			...
			if (!(text14 == ""))
			{
				SetStatusAccount(indexRow, text2 + text12 + (flag ? "- Facebook blocked" : ""));
			}
			else
			{
					...
					while (true)
					{
						try
						{
							if (isResetAdb && lstDevice.Count > 0)
							{
								...
								{
									flag = true;
									for (int num9 = 0; num9 < lstDevice.Count; num9++)
									{
										string deviceByIndex = ADBHelper.GetDeviceByIndex(lstDevice[num9].IndexDevice);
										...
										{
											lstDevice[num9].DeviceId = deviceByIndex;
										}
									}
								}
								isResetAdb = false;
			...
			try
			{
				...
				else
				{
					while (true)
					{
						...
						case 10:
							...
							lock (lock_StartProxy)
							{
								while (!isStop)
								{
									...
									while (!isStop)
									{
										foreach (TMProxy item2 in listTMProxy)
										{
											if (tMProxy == null || item2.daSuDung < tMProxy.daSuDung)
											{
												tMProxy = item2;
											}
										}
						...
						case 8:
							...
							lock (lock_StartProxy)
							{
								while (!isStop)
								{
									...
									while (!isStop)
									{
										foreach (XproxyProxy item3 in listxProxy)
										{
											if (xproxyProxy == null || item3.daSuDung < xproxyProxy.daSuDung)
											{
												xproxyProxy = item3;
											}
										}
						...
						case 7:
							...
							lock (lock_StartProxy)
							{
								while (!isStop)
								{
									...
									while (!isStop)
									{
										foreach (TinsoftProxy item4 in listTinsoft)
										{
											if (tinsoftProxy == null || item4.daSuDung < tinsoftProxy.daSuDung)
											{
												tinsoftProxy = item4;
											}
										}
						...
						int valueInt = setting_InteractGeneral.GetValueInt("nudSoLuongBaiFrom");
						int valueInt2 = setting_InteractGeneral.GetValueInt("nudSoLuongBaiTo");
						int valueInt3 = setting_InteractGeneral.GetValueInt("nudKhoangCachFrom");
						int valueInt4 = setting_InteractGeneral.GetValueInt("nudKhoangCachTo");
						bool valueBool = setting_InteractGeneral.GetValueBool("ckbVanBan");
						bool valueBool2 = setting_InteractGeneral.GetValueBool("ckbAnh");
						int valueInt5 = setting_InteractGeneral.GetValueInt("nudSoAnhFrom");
						int valueInt6 = setting_InteractGeneral.GetValueInt("nudSoAnhTo");
						bool valueBool3 = setting_InteractGeneral.GetValueBool("ckbXoaNguyenLieuDaDung");
						List<string> list = new List<string>();
						if (!valueBool3)
						{
							list = CloneList(lstNoiDung);
						}
						int num7 = rd.Next(valueInt, valueInt2 + 1);
						int num8 = 0;
						for (int l = 0; l < num7; l++)
						{
							try
							{
								...
								DelAllFileInFolder("C:\\Users\\Admin\\Documents\\leidian\\Pictures\\Instagram");
								if (device.CheckExistText("search and explore", "", 10.0) && device.TapByText("\"profile\"", "", 10))
								{
									bool flag8 = false;
									int num9 = 0;
									do
									{
										...
										case 5:
											if (valueBool)
											{
												if (device.TapByText("write a caption…", "", 10))
												{
													item = "";
													if (!valueBool3)
													{
														if (list.Count == 0)
														{
															list = MCommon.Common.CloneList(lstNoiDung);
														}
													...
													lock (lstNoiDung)
													{
														...
														int index = rd.Next(0, lstNoiDung.Count);
														item = lstNoiDung[index];
														lstNoiDung.RemoveAt(index);
														goto IL_1d15;
													}
												}
												flag8 = true;
										...
										case 3:
											if (valueBool2)
											{
												...
												device.DelayTime(1.0);
												if (!device.TapByText("\"select multiple\"", "", 10) || !device.CheckExistText("photo thumbnail", "", 10.0))
												{
													break;
												}
												num8 = rd.Next(valueInt5, valueInt6 + 1);
												if (num8 > 10)
												{
													num8 = 10;
												}
												...
												{
													if (!device.TapByText("\"photo thumbnail\"", "", 10))
													{
														goto end_IL_1f4f;
													}
												}
										...
										case 1:
											{
												if (!device.TapByText("\"create new\"", "", 10) || !device.TapByText("\"feed post\"", "", 10))
												{
													break;
												}
												flag8 = true;
		...
		{
			MCommon.Common.ShowForm(new fDangBai_CauHinh());
		}
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(maxcare.fDangBai));
			...
			bunifuCustomLabel1.Size = new System.Drawing.Size(268, 16);
			bunifuCustomLabel1.TabIndex = 3;
			bunifuCustomLabel1.Text = "Max Instagram Pro - Ti\u0301nh năng Đăng ba\u0300i";
