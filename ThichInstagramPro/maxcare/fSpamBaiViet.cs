	...
	public class fSpamBaiViet : Form
	{
			...
			try
			{
				...
				case "proxy":
				{
					for (int num10 = 0; num10 < dtgvAcc.RowCount; num10++)
					{
						...
						{
							text = text + GetCellAccount(num10, "cProxy").Split('*')[0] + "\r\n";
						}
					}
					break;
				}
		...
		private void ExcuteOneThread(int indexRow, int indexPos, bool isRunSwap, string pathLD)
		{
			...
			try
			{
				...
				else
				{
					while (true)
					{
						...
						string path = FileHelper.GetPathToCurrentFolder() + "\\device\\" + text5 + "\\config";
						string path2 = FileHelper.GetPathToCurrentFolder() + "\\device\\" + cellAccount2 + "\\config";
						if ((text5 != "" && File.Exists(path)) || File.Exists(path2))
						{
							string text7 = "";
							text7 = ((!(text5 != "") || !File.Exists(path)) ? cellAccount2 : text5);
							device.RestoreConfigDevice(text7);
							device.ChangeFileConfig();
						}
						else
						{
						...
						if (isRunSwap)
						{
							...
							if (File.Exists(FileHelper.GetPathToCurrentFolder() + "\\profile\\" + text5 + "\\data.tar.gz"))
							{
								device.LoadStatusLD("Restore data Fb...");
						...
						if (text8.Split('|')[1] == "0")
						{
							num2 = LoginFacebook(device, indexRow, text2);
						}
						...
						case 9:
						{
							...
							for (int m = 0; m < 10; m++)
							{
								text8 = device.OpenInstagramAndCheckStatusLogin();
								if (!(text8.Split('|')[1] == "1"))
								{
									device.DelayTime(1.0);
									continue;
						...
						device.LoadStatusLD("Login success");
						...
						if (setting_InteractGeneral.GetValueBool("ckbGetCookie"))
						{
							...
							cellAccount6 = tokenCookie.Split('|')[0];
							cellAccount5 = tokenCookie.Split('|')[1];
							CommonSQL.UpdateFieldToAccount(cellAccount, "token", cellAccount6);
							SetCellAccount(indexRow, "cToken", cellAccount6);
						...
						default:
						{
							...
							while (num8 < num7)
							{
								...
								while (true)
								{
									...
									{
									case 2:
										device.GotoPageQuick(text9);
										break;
									case 1:
										device.GotoGroupQuick(text9);
										break;
									case 0:
										device.GotoProfileQuick(text9);
										break;
									}
									...
									case 0:
									{
										SetStatusAccount(indexRow, text2 + Language.GetValue("Đang") + $" Spam ba\u0300i viê\u0301t ({num8 + 1}/{num7})...");
										int randomInt = device.GetRandomInt(valueInt3, valueInt4);
										bool flag9;
										if (flag9 = device.WaitForTextAppear(30.0, "profile picture", "page cover photo", "cover photo of group", "community"))
										{
											for (int n = 0; n < randomInt; n++)
											{
												...
												if (valueBool)
												{
													string text11 = device.GetListBoundsByText("double tap and hold to", html).LastOrDefault();
													if (string.IsNullOrEmpty(text11))
													{
														text11 = device.GetListBoundsByText("like button", html).LastOrDefault();
													}
													if (!string.IsNullOrEmpty(text11))
													{
														SetStatusAccount(indexRow, text2 + Language.GetValue("Tha\u0309 ca\u0309m xu\u0301c ba\u0300i viê\u0301t..."));
												...
												{
													string text12 = device.GetListBoundsByText("comment", html).LastOrDefault();
													if (!string.IsNullOrEmpty(text12))
													{
														SetStatusAccount(indexRow, text2 + Language.GetValue("Comment ba\u0300i viê\u0301t..."));
														device.LoadStatusLD("Comment Post");
														bool flag10 = false;
														device.TapByBounds(text12);
														if (valueBool2)
														{
															string text13 = device.GetListBoundsByText("write a", "", 10).LastOrDefault();
															if (!string.IsNullOrEmpty(text13))
															{
																flag10 = device.CheckExistText("\"video\"");
																if (list3.Count == 0)
																{
																	list3 = CloneList(list);
																}
																text10 = list3.OrderBy((string t) => Guid.NewGuid()).First();
																list3.Remove(text10);
														...
														if (valueBool4)
														{
															...
															{
																for (int num10 = 0; num10 < 5; device.DelayTime(1.0), num10++)
																{
																	html = device.GetHtml();
																...
																if (listBoundsByText.Count >= 11)
																{
																	...
																	text14 = (from t in device.GetListBoundsByText("\"photo")
																		orderby Guid.NewGuid()
																		select t).FirstOrDefault();
																}
																else
																{
																	text14 = (from t in device.GetListBoundsByText("\"photo", html)
																		orderby Guid.NewGuid()
																		select t).FirstOrDefault();
																}
			...
			try
			{
				...
				if (flag4)
				{
					string path3 = FileHelper.GetPathToCurrentFolder() + "\\device\\" + text5 + "\\config";
					string path4 = FileHelper.GetPathToCurrentFolder() + "\\device\\" + cellAccount2 + "\\config";
					if ((text5 != "" && File.Exists(path3)) || File.Exists(path4))
					{
						string text16 = text5;
						if (text16 == "")
						{
			...
			if (!(text18 == ""))
			{
				SetStatusAccount(indexRow, text2 + text15 + (flag3 ? "- Lô\u0303i mơ\u0309 link" : "") + (flag2 ? "- Facebook blocked" : ""));
			}
			else
			{
			...
			if (flag && Directory.Exists("profile\\" + text4))
			{
				string text19 = "profile\\" + text4;
				string pathTo = "profile\\" + text5;
				if (!MCommon.Common.MoveFolder(text19, pathTo) && MCommon.Common.CopyFolder(text19, pathTo))
				{
					MCommon.Common.DeleteFolder(text19);
				}
			}
