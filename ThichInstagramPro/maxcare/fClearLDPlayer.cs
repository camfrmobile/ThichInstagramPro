...
namespace maxcare
{
	public class fClearLDPlayer : Form
	{
		private JSON_Settings setting_general;

		private List<string> lstLDPlayerRac = new List<string>();
		...
		private void BtnAdd_Click(object sender, EventArgs e)
		{
			...
			int count = 0;
			LoadStatus(string.Format(Language.GetValue("Đang dọn dẹp LDPlayer {0}/{1}..."), count, lstLDPlayerRac.Count));
			int iThread = 0;
			int maxThread = 20;
			...
			new Thread((ThreadStart)delegate
			{
				...
				{
					int num = 0;
					while (num < lstLDPlayerRac.Count)
					{
						if (iThread < maxThread)
						{
							...
							new Thread((ThreadStart)delegate
							{
								try
								{
									string path = pathLD + "\\vms\\leidian" + lstLDPlayerRac[row];
					...
					{
						Close();
						MessageBoxHelper.ShowMessageBox(Language.GetValue("Đã dọn dẹp LDPlayer xong!"));
					});
				}
				catch
				{
		...
		private List<string> GetListDeviceFromDatabase()
		{
			List<string> list = new List<string>();
			try
			{
				DataTable allAccountFromDatabase = CommonSQL.GetAllAccountFromDatabase("device");
				for (int i = 0; i < allAccountFromDatabase.Rows.Count; i++)
				{
					if (allAccountFromDatabase.Rows[i]["device"].ToString() != "")
					{
						list.Add(allAccountFromDatabase.Rows[i]["device"].ToString());
					}
				}
			}
			catch
		...
		private void OnLoaded(object sender, EventArgs e)
		{
			...
			new Thread((ThreadStart)delegate
			{
				...
				{
					List<string> list = new List<string>();
					string[] directories = Directory.GetDirectories(ConfigHelper.GetPathLDPlayer(1) + "\\vms");
					string[] array = directories;
					foreach (string text in array)
					{
						string text2 = text.Substring(text.LastIndexOf("\\") + 1);
						if (text2.Contains("leidian") && MCommon.Common.IsNumber(text2.Replace("leidian", "")))
						{
							list.Add(text2.Replace("leidian", ""));
						}
					}
					List<string> listDeviceFromDatabase = GetListDeviceFromDatabase();
					lstLDPlayerRac = list.Except(listDeviceFromDatabase).ToList();
					if (lstLDPlayerRac.Count > 0)
					{
						Invoke((Action)delegate
						{
							lblStatus.Text = string.Format(Language.GetValue("Tìm thấy {0} LDPlayer rác!"), lstLDPlayerRac.Count) + "\r\n" + Language.GetValue("Bạn có muốn dọn dẹp?");
							btnAdd.Visible = true;
					...
					{
						MessageBoxHelper.ShowMessageBox(Language.GetValue("Không có LDPlayer rác!"));
						Invoke((Action)delegate
						{
							Close();
						});
		...
		private void InitializeComponent()
		{
			...
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(maxcare.fClearLDPlayer));
			...
			bunifuCustomLabel1.Text = "Dọn dẹp Ldplayer";
			...
			lblStatus.Text = "Đang quét Ldplayer...";
