...
namespace maxcare
{
	public class Comon
	{
		...
		public static bool CheckAvatar(string uid)
		{
			...
			string text = requestHTTP.Request("GET", "https://graph.facebook.com/" + uid + "/picture");
		...
		public static string LoadErrorFromErrorcode(string temp)
		{
			...
			if (temp.Split('|')[0] == "1")
			{
				result = Convert.ToInt32(temp.Split('|')[1]) switch
				{
					400 => "Tài khoản không tồn tại", 
					104 => "Lỗi phần mềm", 
					613 => "Giới hạn lấy token", 
					405 => Language.GetValue("Checkpoint"), 
					401 => "Changed pass", 
					_ => "Lỗi hệ thống", 
				};
			}
		...
		public static string CheckKeyAndLoginPro(string userName, string passWord, string idKey = "", int softIndex = 62)
		{
			if (idKey.Equals(""))
			{
				idKey = GetIdKey();
