...
namespace MCommon
{
	public class CommonChrome
	{
		...
		public static string LoginInstagram(Chrome chrome, string cookie, string URL = "https://www.instagram.com")
		{
			try
			{
				...
				chrome.AddCookieIntoChrome(cookie, ".instagram.com");
				chrome.Refresh();
		...
		public static string LoginInstagram(Chrome chrome, string user, string pass, string fa2, int timeOut = 120)
		{
			...
			try
			{
				chrome.GotoURL("https://www.instagram.com/accounts/login/?source=auth_switcher");
				for (int i = 0; i < 10; i++)
				{
					if (!chrome.CheckIsLive())
					{
					...
					if (!CheckLoginSuccess(chrome))
					{
						if (chrome.CheckExistElement("[name=\"username\"]") != 1 || chrome.CheckExistElement("[name=\"password\"]") != 1)
						{
							if (chrome.CheckExistElement("button[type=\"button\"]") != 1)
							{
								chrome.DelayTime(1.0);
								continue;
							}
				...
				while (Environment.TickCount - tickCount < timeOut * 1000 && chrome.CheckIsLive())
				{
					switch (chrome.CheckExistElements(0.0, "[name=\"username\"]", "[role=\"dialog\"]>div>div>div:nth-child(3)>button:nth-child(2)", "[name=\"verificationCode\"]"))
					{
					default:
					...
					case 1:
					{
						string text = chrome.ExecuteScript("var out='';var x=document.querySelector('#slfErrorAlert');if(x!=null) out=x.innerText;return out;").ToString();
						if (text != "")
						{
							return text;
						}
					...
					case 3:
						{
							...
							else if (chrome.CheckExistElement("[data-visualcompletion=\"loading-state\"]") != 1)
							{
								num3++;
								if (num3 > 2)
								{
								...
								chrome.SendKeys(2, "verificationCode", totp);
		...
		{
			if (!chrome.GetURL().StartsWith("https://www.instagram.com/"))
			{
				chrome.GotoURL("https://www.instagram.com/");
			}
			return chrome.CheckExistElements(0.0, "[href=\"/direct/inbox/\"]", "[href=\"/explore/\"]", "[href=\"/accounts/activity/\"]") > 0;
		}
		...
		public static bool CheckCheckpoint(Chrome chrome)
		{
			...
			return chrome.CheckExistElements(0.0, "#recaptcha-input") > 0;
		}
		...
		public static int LoginFacebookUsingUidPass(Chrome chrome, string uid, string pass, string fa2 = "", string URL = "https://www.facebook.com", int tocDoGoVanBan = 0)
		{
			...
			try
			{
				...
				else
				{
					...
					if (num6 != -2)
					{
						if (num6 != 1)
						{
							...
							{
								chrome.DelayThaoTacNho();
								if (chrome.CheckExistElements(3.0, "#m_login_password", "[name=\"pass\"]") == 1)
								{
									_ = "#m_login_password";
								}
								else
				...
				while (chrome.CheckExistElement("#checkpointSubmitButton-actual-button", 3.0) == 1)
				{
					...
					if (num4 != 0)
					{
						string attributeValue = ((num4 == 1) ? "#approvals_code" : "[name=\"approvals_code\"]");
						if (fa2.Trim() != "")
						{
							string text = "";
							for (int i = 0; i < 10; i++)
							...
							if (text != "")
							{
								chrome.SendKeys(4, attributeValue, text, 0.1);
								chrome.DelayThaoTacNho(-1);
								chrome.Click(1, "checkpointSubmitButton-actual-button");
								chrome.DelayThaoTacNho();
							}
