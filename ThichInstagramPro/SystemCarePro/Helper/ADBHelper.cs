...
namespace SystemCarePro.Helper
{
	public class ADBHelper
	{
		...
		public static string PushFile(string deviceId, string fromFilePath, string toFilePath)
		{
			try
			{
				return RunCMD(deviceId, string.Format(ADBCommands.PUSH_FILE, fromFilePath, toFilePath));
			}
			catch
			{
			}
		...
		public static string GetDeviceByIndex(int IndexDevice)
		{
			...
			try
			{
				...
				List<string> list = devices.Where((string x) => x == deviceId1 || x == deviceId2).ToList();
				if (list.Count > 0)
				{
					result = list[0];
				}
